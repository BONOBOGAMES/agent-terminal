...
# HA Smart Context — generates AGENTS.md with Home Assistant context.
# Grok Build loads ~/.grok/AGENTS.md as global project instructions.
#
# Usage:
...
# Global Grok instructions path (HOME is /data/home in the add-on)
OUTPUT_FILE="${HOME}/.grok/AGENTS.md"
FULL_MODE=false
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help)
            ...
            echo ""
            echo "Generate Home Assistant context for Grok Build sessions."
            echo "Writes AGENTS.md that Grok automatically loads from ~/.grok/."
            echo ""
            echo "Options:"
            echo "  --full       Include entity ID listings (detailed mode)"
            echo "  --output F   Write to file F instead of \$HOME/.grok/AGENTS.md"
...
> Auto-generated by Agent Terminal. Run \`ha-context\` to refresh.
> Last updated: ${timestamp}

You are working inside a Home Assistant add-on terminal. Working directory is
\`/config\` (Home Assistant configuration). Also mounted: \`/addon_configs\`,
\`/share\`. Persistent agent state lives under \`/data\`.
...
If ha-mcp MCP tools are configured, prefer those for device control and
automation management when available.
APIREF
